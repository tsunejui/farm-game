...
| `TileMap` | `World/TileMap.cs` | Map data, tile queries, rendering |
| `MapGenerator` | `World/MapGenerator.cs` | Procedural map creation |
...
3. `TileMap.Draw` — renders only tiles visible in the camera viewport
...
1. Add the value to the `TileType` enum in `World/TileType.cs`
2. Add its color in the `GetTileColor` switch expression in `World/TileMap.cs`
3. Set passability in `IsPassable` in `World/TileMap.cs` if needed
4. Place it in the map via `World/MapGenerator.cs`
